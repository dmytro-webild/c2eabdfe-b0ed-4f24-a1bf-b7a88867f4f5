  ...
  const makeEditable = (element, clickEvent) => {
    ...
    if (clickEvent && element.childNodes.length > 0) {
      try {
        ...
        console.warn('[CONTACT US ] Could not set caret position:', e);
      }
    }
  };
  ...
  const handleMessage = (e) => {
        ...
        if (savedChanges) {
          ...
          changes.forEach(change => {
            try {
              ...
              console.warn('[CONTACT US ] Failed to revert change:', err);
            }
          });
        }
        clearLocalChanges();
      } catch (error) {
        console.error('[CONTACT US ] Failed to cancel changes:', error);
      }
      return;
    }
      ...
      try {
        ...
        if (selectedElement && isTextElement(selectedElement)) {
          ...
          try {
            ...
            console.warn('[CONTACT US ] Invalid selector:', selector);
          }
        }
    ...
    if (e.data.type === 'webild-update-button') {
      ...
      try {
        ...
        console.error('[CONTACT US ] Invalid selector for button update:', selector, error);
      }
      return;
    }
